...
import { usePomodoroBroadcast } from '../../src/common/broadcast/usePomodoroBroadcast'
...
describe('pomodoro broadcast', () => {
  it('receives channel messages, deduplicates storage fallback, and cleans up', async () => {
    ...
    const sender = senderScope.run(() => usePomodoroBroadcast())!
    const receiver = receiverScope.run(() => usePomodoroBroadcast({ onChanged }))!
    await nextTick()
    await nextTick()
    ...
    try {
      ...
      const key = 'pomodoroSceneStore_storage'
