...
<template>
  <div class="countdown-list-config mx-auto flex w-full max-w-2xl flex-col gap-4 py-2">
      ...
      <CardContent class="flex flex-col gap-5 px-5">
        <div class="flex items-center justify-between gap-4">
          <div class="grid gap-1.5">
            <Label for="countdown-list-global-theme">使用全局主题</Label>
            <p class="text-xs text-muted-foreground">
              跟随桌面组件的统一外观
            </p>
          </div>
          <Switch id="countdown-list-global-theme" v-model="widgetTheme.useGlobalTheme" />
        </div>
        <template v-if="!widgetTheme.useGlobalTheme">
          <Separator />
          <div v-for="field in colorFields" :key="field.key" class="config-row">
            <Label :for="`countdown-list-${field.key}`">{{ field.label }}</Label>
            <ThemeColorPicker :id="`countdown-list-${field.key}`" v-model="widgetTheme[field.key]" :label="field.label" />
          </div>
        </template>
      </CardContent>
    </Card>
