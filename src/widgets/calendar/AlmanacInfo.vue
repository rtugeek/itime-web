...
const suitableActivities = ['出行', '谈生意', '签约', '装修', '开业', '搬家', '结婚']
const avoidActivities = ['动土', '开工', '置业', '乔迁', '开张', '祭祀', '动武']
const festivals = ['清明节', '春分', '谷雨']
const currentDate = ref('')
const lunarDate = ref('农历癸卯年三月初六')
const weekday = ref('')
const festival = ref('')
const suitable = ref('')
const avoid = ref('')
...
function updateCalendar() {
  ...
  festival.value = festivals[Math.floor(Math.random() * festivals.length)]

  const randomSuitable = [...suitableActivities]
    .sort(() => 0.5 - Math.random())
    .slice(0, 3)
  const randomAvoid = [...avoidActivities]
    .sort(() => 0.5 - Math.random())
    .slice(0, 3)

  suitable.value = randomSuitable.join('、')
  avoid.value = randomAvoid.join('、')
}
