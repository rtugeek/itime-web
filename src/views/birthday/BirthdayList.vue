...
useSortable(listRef, store.birthdayList, {
  animation: 150,
  onEnd: async () => {
    await delay(300)
    await store.saveAll(store.birthdayList)
  },
})
  ...
  <main class="min-w-0 flex-1 p-3 sm:p-4">
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-8">
      ...
      <section aria-labelledby="birthday-list-title" class="space-y-4">
        <ul v-if="birthdayList.length" ref="listRef" class="grid list-none grid-cols-1 gap-4 p-0 lg:grid-cols-2">
          <li
              ...
              <div class="min-w-0 flex-1 space-y-2">
                ...
                <div class="flex min-w-0 flex-wrap items-center gap-2 text-xs leading-5 text-muted-foreground">
                  <span class="inline-flex min-w-0 items-center gap-1.5">
                    ...
                    <span class="break-words">{{ new Birthday(birthday).format() }}</span>
                  </span>
                  <span v-if="birthday.dateType === 1" class="inline-flex min-w-0 items-center gap-1.5">
                    ...
                    <span class="break-words">{{ new Birthday(birthday).format(0) }}</span>
                  </span>
                  <span class="inline-flex items-center gap-1.5">
                    <Cake class="size-4 shrink-0" aria-hidden="true" />
                    <span>{{ Math.max(0, new Date().getFullYear() - new Birthday(birthday).getSourceSolarDate().getFullYear()) }}岁</span>
                  </span>
                </div>
              </div>
              <div class="shrink-0 text-right">
                <span v-if="new Birthday(birthday).isToday()" class="text-lg font-semibold tracking-tight">今天生日</span>
                <div v-else class="flex items-baseline justify-end gap-1">
                  <span class="text-xs text-muted-foreground">还有</span>
                  <span class="text-3xl font-semibold tracking-tight tabular-nums">{{ new Birthday(birthday).countdown() }}</span>
