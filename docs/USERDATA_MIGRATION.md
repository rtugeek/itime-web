...
> 当前已在 `src/main.ts` 挂载应用前调用 `src/utils/LegacyMigration.ts`，逐条导入四模块到 `userdata-v2.userDatas`。下文仍包含完整迁移方案建议，不能将所有建议视为已实现。实际实现范围及限制见下节。

### 当前自动迁移实现（2026-09-18）

- 读取本文列出的四个 IndexedDB 旧库，包含软删除记录；完整 JSON 包含番茄历史。先触发备份下载，再转换并在同一事务中逐条插入 UserData、保存 JSON 和完成标记。
- 按用户确认沿用有效数字 `userId`，缺失或 `0` 为未绑定；无效账号不自动转成游客。保留本地 Date 时间、删除标记和 Todo 完成状态；兼容 BirthdayWrapper、旧 periodType 到 RRULE 的转换。
- 无效时间（含未声明时区的字符串）、排序、部分必填业务字段及 ID 冲突会阻止迁移。失败时保留旧源并在启动页显示错误，应用 Store 和自动同步不会启动。
- 目前保留旧 ID 的字符串形式，场景与历史、组件配置的引用不变；源记录之间或与新库已有记录的 ID 冲突均报错，不能仅凭同类型同 ID 跳过。尚未实现跨设备统一 ID 映射、自动合并及引用重写，不能宣称跨设备去重已完成。
- **迁移成功后保留全部旧版本数据，番茄历史库 `pomodoro` 不清空、不改写。** 后续启动根据完成标记跳过迁移，不重复导入，也不补做旧库清理。JSON 备份保存在新库 `migrations` 表。
- 更早版本的 localStorage/WebSQL 驱动、孤立历史引用审计、组件配置备份和跨旧版本窗口的写入互斥尚未实现。执行时应关闭旧版本窗口。此客户端迁移不代替服务端迁移及跨设备对账。
