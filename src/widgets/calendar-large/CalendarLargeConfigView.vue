...
<template>
  <div class="calendar-large-config mx-auto flex w-full max-w-2xl flex-col gap-4 py-2">
      ...
      <CardContent class="flex flex-col gap-5 px-5">
        <div class="flex items-center justify-between gap-4">
          <div class="grid gap-1.5">
            <Label for="calendar-large-global-theme">使用全局主题</Label>
            <p class="text-xs text-muted-foreground">
              跟随桌面组件的统一外观
            </p>
          </div>
          <Switch id="calendar-large-global-theme" v-model="widgetTheme.useGlobalTheme" />
        </div>
        <template v-if="!widgetTheme.useGlobalTheme">
          <Separator />
          <div v-for="field in colorFields" :key="field.key" class="config-row">
            <Label :for="`calendar-large-${field.key}`">{{ field.label }}</Label>
            <ThemeColorPicker :id="`calendar-large-${field.key}`" v-model="widgetTheme[field.key]" :label="field.label" />
          </div>
          <Separator />
          <div class="grid gap-4 pb-1">
            <div class="flex items-center justify-between">
              <Label for="calendar-large-border-radius">圆角</Label>
              <span class="rounded-md bg-muted px-2 py-1 text-xs tabular-nums text-muted-foreground">{{ borderRadius[0] }} px</span>
            </div>
            <Slider id="calendar-large-border-radius" v-model="borderRadius" :min="0" :max="50" :step="1" aria-label="圆角" />
          </div>
        </template>
      </CardContent>
    </Card>
